...
async function handler(
  ...
  const [app] = await Promise.all([MongoApp.findById(appId, 'type').lean()]);
  if (!app) {
  ...
  const fieldMap = {
    ...
    [GetChatTypeEnum.outLink]: `dataId obj value userGoodFeedback userBadFeedback adminFeedback ${
      isPlugin ? `${DispatchNodeResponseKeyEnum.nodeResponse}` : ''
    } `,
  ...
  return {
    list: isPlugin ? histories : transformPreviewHistories(histories, true),
    total
  };
}
