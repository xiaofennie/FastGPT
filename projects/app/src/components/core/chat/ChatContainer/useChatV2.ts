...
import { getChatRecordsV2 } from '@/web/core/chat/api';
...
export const useChatV2 = (params?: { chatId?: string; appId: string; type?: GetChatTypeEnum }) => {
  ...
  const {
    ...
    async (data: getPaginationRecordsBody): Promise<PaginationResponse<ChatSiteItemType>> => {
      const res = await getChatRecordsV2(data);
